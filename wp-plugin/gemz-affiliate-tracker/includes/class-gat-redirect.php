...
class GAT_Redirect {
	...
	public static function add_rewrite_rule() {
		add_rewrite_rule( '^go/([a-zA-Z0-9_-]+)/?$', 'index.php?gat_code=$matches[1]', 'top' );
	}
	 ...
	 * partner's real affiliate URL. Unknown codes fall through untouched
	 * (WordPress will 404 normally, no click is logged).
	 */
	public static function handle_redirect() {
		$code = get_query_var( 'gat_code' );
		if ( empty( $code ) ) {
			return;
		}
		...
		// No destination configured yet (affiliate link not approved):
