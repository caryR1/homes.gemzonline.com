...
class GAT_REST {
	...
	public static function register_routes() {
		register_rest_route( 'gat/v1', '/partners', array(
			'methods'             => 'GET',
			'callback'            => array( __CLASS__, 'list_partners' ),
			'permission_callback' => array( __CLASS__, 'permission_check' ),
		) );

		register_rest_route( 'gat/v1', '/partners/(?P<id>\d+)', array(
			'methods'             => 'POST',
		...
		register_rest_route( 'gat/v1', '/codes', array(
			'methods'             => 'GET',
			'callback'            => array( __CLASS__, 'list_codes' ),
			'permission_callback' => array( __CLASS__, 'permission_check' ),
		) );

		register_rest_route( 'gat/v1', '/affiliates', array(
			'methods'             => 'GET',
	...
	public static function list_partners() {
		global $wpdb;
		$rows = $wpdb->get_results( 'SELECT * FROM ' . GAT_DB::table( 'partners' ) . ' ORDER BY name ASC', ARRAY_A );
		foreach ( $rows as &$r ) {
	...
	public static function update_partner( WP_REST_Request $request ) {
		...
		$table = GAT_DB::table( 'partners' );

		$existing = $wpdb->get_row( $wpdb->prepare( "SELECT id FROM {$table} WHERE id = %d", $id ) );
		if ( ! $existing ) {
			return new WP_Error( 'gat_not_found', 'Partner not found.', array( 'status' => 404 ) );
		}
		...
		if ( empty( $data ) ) {
			return new WP_Error( 'gat_no_fields', 'No recognized fields to update.', array( 'status' => 400 ) );
		}
	...
	public static function list_affiliates() {
		...
		foreach ( $rows as &$r ) {
			...
			$r['payment_details'] = $r['wp_user_id'] ? get_user_meta( $r['wp_user_id'], 'gat_payment_details', true ) : '';
		}
		return new WP_REST_Response( $rows, 200 );
	}
}
